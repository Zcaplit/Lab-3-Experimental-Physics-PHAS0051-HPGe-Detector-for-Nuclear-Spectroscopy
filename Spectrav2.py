...
##=======================Analysis========================##
    ...
    #label the peaks
    def label_peaks(self,axs,n1 = 0, n2 = None, E_err = True, tcolor = 'black',fontsize = 12,halign ='center',valign = 'bottom',perc = 2):
        ...
        if E_err == True:
            ...
            if n2 == None:
                for i,p in enumerate(self.Samplepeaks[n1:,0]):
                    top = np.max(self.sample[int(self.ch_of(p))-3:int(self.ch_of(p))+3])
                    axs.text(p,top,"$peak_{0}$\n${1:.{perc}f}\pm{2:.{perc}f}$keV".format(n1+i+1,self.Samplepeaks[n1+i,0],self.Samplepeaks[i,1],perc=perc),color = tcolor,fontsize=fontsize,horizontalalignment=halign,verticalalignment=valign)
            else:
                for i,p in enumerate(self.Samplepeaks[n1:n2,0]):
                    top = np.max(self.sample[int(self.ch_of(p))-3:int(self.ch_of(p))+3])
                    axs.text(p,top,"$peak_{0}$\n${1:.{perc}f}\pm{2:.{perc}f}$keV".format(n1+i+1,self.Samplepeaks[n1+i,0],self.Samplepeaks[i,1],perc=perc),color = tcolor,fontsize=fontsize,horizontalalignment=halign,verticalalignment=valign)
